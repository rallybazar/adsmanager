...
* Author: Thomas PAPIN (webmaster@Juloa.com)
*/
...
class AdsManagerFavoriteTab extends cbTabHandler {
  ...
  function getDisplayTab($tab,$user,$ui) {
	...
	if ($favoriteads == true) 
		require_once(JPATH_ROOT.'/components/com_adsmanager/views/favorites/view.html.php');
	else
	...
	if ($favoriteads == true) {
		if (is_file($templateDir.'/html/com_adsmanager/favorites/default.php')) {
			$config['template_path'] = JPATH_BASE.'/templates/' . $app->getTemplate().'/html/com_adsmanager/favorites';
		} else {
			$config['template_path'] = JPATH_BASE.'/components/com_adsmanager/views/favorites/tmpl';
		}
		$view = new AdsmanagerViewFavorites($config);
	} else {
	...
	$pagination = new JPagination2($total, $limitstart, $limit,"index.php?option=com_comprofiler&tab=AdsmanagerFavoriteTab&limit=$limit".$userId);
	...
	$banners = false;
	$view->assignRef('banners', $banners);

...
class plgAdsmanagercontentSocial extends JPlugin {
    ...
    private function getContent($content){
        ...
        $html   .= $this->getMix($this->params, $url, $title);
    ...
    private function getTwitter($params, $url, $title){
        ...
        if($params->get("twitterButton")) {
            $html = '<iframe
                src="https://platform.twitter.com/widgets/tweet_button.html?size='.$params->get("twitterSize").'&url='.urlencode($url).'&via=' . $params->get("twitterName") . '&text=' . urlencode($title) . '&hashtags=example%2Cdemo"
                width="140"
                height="37"
    ...
    private function getMix($params, $url, $title){

        $html = "";
        if($params->get("mixButton")) {
			
			$mixTypeLabel = '';
			if($params->get("mixType") == 2) {
				$mixTypeLabel = ' layout="2"';
			}

            $html = '
            <div class="itp-share-mix">
			<mix:badge' . $mixTypeLabel . ' location="' . rawurlencode($url) . '"></mix:badge>
            <script async defer src="https://mix.com/badge/loader"></script>
            </div>
            ';
        }
    ...
    private function getFacebookShareMe($params, $url, $title){
            ...
            if($params->get("facebookShareMeButton")) {
                ...
                <iframe
				src="https://www.facebook.com/plugins/share_button.php?href='.urlencode($url).'&layout=button_count&size='.$params->get("facebookShareMeType").'&mobile_iframe=true&width=103&height=20&appId"
				width="103"
				height="20"
				style="border:none;overflow:hidden;float:left;margin-top:5px;"
				scrolling="no"
				frameborder="0"

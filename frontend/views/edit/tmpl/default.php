            ...
            <div class="control-group">
                ...
                <?php
                    switch($this->conf->single_category_selection_type) {
                        ...
                        case 'combobox':
                                JHTMLAdsmanagerCategory::displayComboboxCategories($selectid ,$this->cats,$this->catid,array("root_allowed"=>$this->conf->root_allowed,"display_price"=>true),array("required"=>"","class"=>"input-large text-center chosen-select"));break;
                            break;
                        case 'cascade':
...
jQ().ready(function() {
	...
	jQ('#adminForm').validate().settings.ignore = ".ignore_validation";
});
	...
	{ 
		if (strpos($field->catsid, ",-1,") === false)
		{
			...
			if (($field->type == "multicheckbox")||($field->type == "multiselect"))
				$name .= "[]";
		?>
		var input = document.getElementById('f<?php echo $name;?>');
		    ...
		    ((singlecat == 1)&&(document.adminForm.category.value == 0)))
		{
			if (input != null) {
				...
				input.addClass('ignore_validation');
			}
			if (trzone != null) {
			...
			for (var i=0; i < length; i++) {
				...
				if (test != -1)
				{
					if (input != null) {
						...
						input.removeClass('ignore_validation');
					}
					if (trzone != null) {
...
function checkdependency(child,parentname,parentvalues,cleanValue) {
	...
	if (jQ('input[name="'+parentname+'"]').is(':checkbox')) {
		...
		if (jQ('input[name="'+parentname+'"]').is(':checked')) {
			...
			jQ('#adminForm #f'+child).removeClass('ignore_validation');
			jQ('#adminForm #row_'+child).show();
		}
		else {
			jQ('#adminForm #f'+child).hide();
			jQ('#adminForm #f'+child).addClass('ignore_validation');
...
//needed to avoid a weird issue with the tooltip

...
class AdsmanagerViewResult extends TView
{
	...
	//TODO	JFactory::getApplication()->allowCache(true);
		...
		$fieldsMapExist = $fieldmodel->getFieldsByType('map');
		if(!empty($fieldsMapExist)) {
			$geolocalisationEnabled = true;
		} else {
		...
		if($geolocalisationEnabled) {
			//getLatLon save it in session to avoid error when changing page
			$lat = $app->getUserStateFromRequest('com_adsmanager.front_content.posLat','posLat',0,'float');
			$lon = $app->getUserStateFromRequest('com_adsmanager.front_content.posLon','posLon',0,'float');
		} else {
			$lat = 0;
			$lon = 0;
		}
		...
		if($geolocalisationEnabled) {
			$total = $contentmodel->getNbContents($filters,0,0,$filter_order,$lat,$lon,$fieldsMapExist[0]->name);
			$contents = $contentmodel->getContents($filters,$limitstart, $limit,$filter_order,$filter_order_dir,0,0,$lat,$lon,$fieldsMapExist[0]->name);
		} else {
			$total = $contentmodel->getNbContents($filters);
			$contents = $contentmodel->getContents($filters,$limitstart, $limit,$filter_order,$filter_order_dir,0,0);
		}
		...
		$this->assignRef('geolocalisationEnabled',$geolocalisationEnabled);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $app->redirect(JRoute::_('index.php?option=com_adsmanager&view=result'));
        }
		
		//If advertising is enabled
		//Check if Bruce is installed and enabled, if not to nothing
		if(isset($conf->advertising_enabled) && $conf->advertising_enabled == 1) {
			if (!JComponentHelper::getComponent('bruce', true)->enabled) {
				include_once(JPATH_ROOT.'/components/com_bruce/lib/core.php');
				if(isset($conf->advertising_banners_orders) && $conf->advertising_banners_orders != '') {
					$order = $conf->advertising_banners_orders;
				} else {
					$order = 0;
				}
				if(isset($conf->advertising_category_id) && $conf->advertising_category_id != '') {
					$banners = Bruce::getBanners($conf->advertising_category_id,$order);
				} else {
					$banners = Bruce::getBanners(false,$order);
				}
				...
				if(empty($banners)) {
					...
					$this->nbAdsBetweenBanners = $conf->advertising_number_ads_between;
					$this->displayAllBannersAfterAds = $conf->advertising_display_all_banners_after_ads;
				}
			}
		} else {
			$this->banners = false;

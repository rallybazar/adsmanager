	...
	<div class="row-fluid">
        ...
        <div class="span4 adsmanager-images text-center">
			<?php
				...
				{
					$imagePath = JURI_IMAGES_FOLDER."/".$img->image;
					$thumbnail = JURI_IMAGES_FOLDER."/".$img->thumbnail."?".filemtime(JPATH_IMAGES_FOLDER."/".$img->thumbnail);
					$image = JURI_IMAGES_FOLDER."/".$img->image."?".filemtime(JPATH_IMAGES_FOLDER."/".$img->image);

					if (isset($this->conf->display_rotate_image) && $this->conf->display_rotate_image == 1 && ($this->content->userid != 0 && $this->userid == $this->content->userid)) {
						echo "<a class='rotateimage-gallery' data-image='".$cpt."' data-src='".$img->image."' data-path='".$imagePath."' href ='#'>".JText::_("ADSMANAGER_ROTATE_IMAGE")."</a>";
					}
					switch($this->conf->image_display)
				    {
				    	case 'popup':
							echo "<a href=\"javascript:popup('$image');\"><img class='content-image-".$cpt."' src='".$thumbnail."' alt=\"".htmlspecialchars($this->content->ad_headline)."\" /></a>";
							break;
						case 'lightbox':
						case 'lytebox':
							echo "<a href='".$image."' rel='lytebox[roadtrip".$this->content->id."]'><img class='content-image-".$cpt."' src='".$thumbnail."' alt=\"".htmlspecialchars($this->content->ad_headline)."\" /></a>"; 
							break;
						case 'highslide':
							echo "<a id='thumb".$this->content->id."' class='highslide' onclick='return hs.expand (this)' href='".$image."'><img class='content-image-".$cpt."' src='".$thumbnail."' alt=\"".htmlspecialchars($this->content->ad_headline)."\" /></a>";
							break;
						case 'default':	
						default:
							echo "<a href='".$image."' target='_blank'><img class='content-image-".$cpt."' src='".$thumbnail."' alt=\"".htmlspecialchars($this->content->ad_headline)."\" /></a>";
							break;
					}
					$cpt++;
				}
				...
				<script type='text/javascript'>
					jQuery(document).ready(function() {
						jQuery('.rotateimage-gallery').click(function() {
							...
							jQuery.get('".JRoute::_('index.php?option=com_adsmanager&task=rotate&id='.$this->content->id, false)."', {src: src, path: path}, function(data) {

...
class TRoute
{
	...
	static function _($url, $xhtml = true, $ssl = null)
	{
		...
		parse_str( JString::str_ireplace( 'index.php?' , '' , $url  ), $result );
		...
		if(empty($itemid[$view]))
		{
			...
			if ($originalview == "details") {
				...
				$viewId =TRoute::_getResultViewItemid($catid,$urloption);
			} else if ($originalview == "list") {
				$viewId =TRoute::_getListViewItemid($catid,$urloption);
			} else {
	...
	static function _getListViewItemid($catid,$urloption='com_adsmanager')
	{
		static $resultsitemid = array();
		...
		if(empty($resultsitemid[$catid]))
		{
			...
			if ($val == null) {
				...
				if (version_compare(JVERSION,'2.5','<')) {
					$query	= 'SELECT id FROM #__menu '
							. 'WHERE link LIKE '. $url. " AND link LIKE '%catid=0%' "
							. 'AND published =' . $db->Quote( 1 ) . ' '
									. 'AND type =' . $db->Quote( 'component' );
					$db->setQuery($query);
					$val = $db->loadResult();

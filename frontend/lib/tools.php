...
class TTools {
	...
	static function redirectToLogin($returnurl="") {
		...
		$returnurl = urlencode((base64_encode(TRoute::_($returnurl,false))));
    ...
    static function loadModule($module, $title, $style = 'none', $strict = false)
	{
        $mods[$module] = '';
        ...
        if($strict) {
		    //If strict is true then we check if the module with the exact title exist

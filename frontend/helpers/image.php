...
class TImage {
	...
	static public function displayImageUploader($conf,$content,$adext) {
					...
					if (isset($content->images)) {
						...
						foreach($content->images as $img) {
							...
							echo "<li class='ui-state-default' id='li_img_$index'>";
                                ...
                                if (@$config->deleteimagemode == "multiple" && JFactory::getApplication()->isAdmin() == true) {
									echo "<input type='checkbox' onChange='selectImage($index)' />".JText::_('ADSMANAGER_SELECT');
								} else {
	                            	echo "<a href='#' name='cb_image$i' onClick='removeImage($index);return false;' value='1' title='image' >".JText::_('ADSMANAGER_CONTENT_DELETE_IMAGE')."</a>";  
                                }
                                echo "</div>";
							echo "</li>";
							$currentnbimages++;
				...
				<?php if (@$config->deleteimagemode == "multiple" && JFactory::getApplication()->isAdmin() == true) { ?>
				<input type="button" class="btn btn-danger" onClick='deleteImages();return false;' value="<?php echo JText::_('ADSMANAGER_DELETE_IMAGES') ?>" />
				<?php } ?>
			...
			var upload_url = <?php echo json_encode(JURI::root().'index.php?option=com_adsmanager&task=upload')?>;
			...
			function selectTmpImage(fileid) {
				if (selectedTmpImages.indexOf(fileid) == -1) {
					selectedTmpImages.push(fileid);
				} else {
					var index = selectedTmpImages.indexOf(fileid);
					selectedTmpImages.splice(index, 1);				
				}
			}
			function selectImage(fileid) {
				if (selectedImages.indexOf(fileid) == -1) {
					selectedImages.push(fileid);
				} else {
					var index = selectedImages.indexOf(fileid);
			...
			function deleteImages() {
				...
				for(i=0;i<selectedTmpImages.length;i++) {
					removeImage(selectedTmpImages[i]);
				}
				selectedTmpImages = []
				selectedImages = []
			}
			...
			function removeTmpImage(fileid){
				<?php if (@$config->deleteimagemode != "multiple" ) { ?>
				...
				jQ("#imagesupload").pluploadQueue({
					...
					init : {
						...
						FileUploaded: function(up, file,info) {
                            ...
                            <?php if (@$config->deleteimagemode == "multiple" ) { ?>
                            	html += "<input type='checkbox' onChange='selectTmpImage(\""+file.id+"\")' /><?php echo JText::_('ADSMANAGER_SELECTED');?>";
							<?php } else { ?>

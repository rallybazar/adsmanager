...
class JHTMLAdsmanagerField
{
	...
	function __construct($conf,$field_values,$mode,$plugins) {
		$this->conf = $conf;
					...
					if ($value != "")
					{
						switch($this->conf->email_display) {
                            case 3:
                                ...
                                if (is_file($templateDir.'/html/com_adsmanager/message/default.php')) {
                                    $pathView = $templateDir.'/html/com_adsmanager/message/default.php';
                                } else {
                                    $pathView = JPATH_ROOT.'/components/com_adsmanager/views/message/tmpl/default.php';
                                }
                                ob_start();
    ...
    function getJoomlaFieldValues($jNameField)
	{
        $user = JFactory::getUser();
		$this->_db->setQuery( "SELECT profile_value as fieldvalue FROM #__user_profiles WHERE profile_key = 'profile.".$jNameField."' AND user_id = ".(int)$user->id." ORDER by ordering ");
		$jfieldvalue = $this->_db->loadResult();
	...
	function showFieldForm($field,$content,$default)
	{
        ...
        if($value == "") {
            if(isset($field->options->joomla_field) && $field->options->joomla_field != '-1') {
                $value = $this->getJoomlaFieldValues($field->options->joomla_field);
            }
        }
		...
		switch($field->type)
		{
			case 'checkbox':
				if ($field->required == 1)
					...
					$mosReq = "";
				if ($value == 1)
					$return .= "<input class='inputbox' type='checkbox' $mosReq  checked='checked' id='f$name' name='$name' value='1' />\n";
				else
					$return .= "<input class='inputbox' type='checkbox' $mosReq  name='$name' id='f$name' value='1' />\n";
                break;
			case 'multicheckbox':
			case 'multicheckboximage':
					...
					for ($j=0 ; $j < $field->cols;$j++)
					{
						...
						if (isset($values[$k]->fieldtitle))
						{
							...
							if ((strpos($value, ",".$fieldvalue.",") === false) &&
								...
								(strpos($value, "|*|".$fieldtitle) === false) &&
								($value !=  $fieldtitle))
								$return .= "<input class='inputbox' type='checkbox' $mosReq  id='f".$name."[]' name='".$name."[]' value=\"".htmlspecialchars($fieldvalue)."\" />&nbsp;$fieldtitle&nbsp;\n";
							else
								$return .= "<input class='inputbox' type='checkbox' $mosReq  id='f".$name."[]' checked='checked' name='".$name."[]' value=\"".htmlspecialchars($fieldvalue)."\" />&nbsp;$fieldtitle&nbsp;\n";
							$return .= "</label>";
						}
						$return .= "</td>";
						$k++;
			...
			case 'select':
				...
				$return .= "</select>";
				$return .= "<script>jQ('#f".$name."').chosen({
                                                no_results_text: ".json_encode(JText::_('ADSMANAGER_SELECT_CATEGORY_NO_RESULT')).",
                                                placeholder_text_single: ".json_encode($placeholder)."
                                            });</script>";
				break;
			...
			case 'multiselect':
				...
				$return .= "<script>jQ('#f".$name."[]').chosen({
                            no_results_text: ".json_encode(JText::_('ADSMANAGER_SELECT_CATEGORY_NO_RESULT')).",
                            placeholder_text_multiple: ".json_encode($placeholder)."
                           });</script>";
				break;
					...
					for ($j=0 ; $j < $field->cols;$j++)
					{
						...
						if (isset($values[$k]->fieldtitle))
						{
							...
							if (($value == $fieldvalue)||($value == $fieldtitle))
								$return .= "<input type='radio' $mosReq id='f$name' name='$name'  value=\"".htmlspecialchars($fieldvalue)."\" checked='checked' />&nbsp;$fieldtitle&nbsp;\n";
							else
								$return .= "<input type='radio' $mosReq id='f$name' name='$name'  value=\"".htmlspecialchars($fieldvalue)."\" />&nbsp;$fieldtitle&nbsp;\n";
                            $return .= "</label>";

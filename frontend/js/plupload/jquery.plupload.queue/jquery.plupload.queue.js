	...
	$.fn.pluploadQueue = function(settings) {
		if (settings) {
			this.each(function() {
				...
				function updateList() {
					var wrapId = target.attr('id');
					var fileList = $('ul#'+wrapId+'_filelist', target).html(''), inputCount = 0, inputHTML;
					inputHTML = '';
					...
					$.each(uploader.files, function(i, file) {
						...
						$('#'+wrapId+'-pluploadfield').html(inputHTML);
					...
					if (uploader.total.queued === 0) {
						...
						$('span.plupload_add_text', target).text(_(label));
					} else {

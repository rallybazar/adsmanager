...
L.Control.GeoSearch = L.Control.extend({
    ...
    geosearch: function (qry) {
        ...
        try {
            ...
            //console.log(typeof provider.GetLocations == 'function');
    ...
    _processResults: function(results, qry) {
        if (results.length > 0) {
            ...
            //HACK, here we need to manage the result
            var fieldid = jQuery('#openmap_fieldid').val();
            jQuery('input#openmap_address'+fieldid).val(results[0].Label);
            jQuery('input#openmap_lat'+fieldid).val(results[0].Y);
            jQuery('input#openmap_lng'+fieldid).val(results[0].X);
            this.cancelSearch();
        } else {

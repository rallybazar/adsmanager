...
class AdsmanagerTableField extends JTable
{
   ...
   var $published = 1;
    ...
    function saveContent($post,$plugins)
    {
		...
		$needSpecificColumnType = false;
    	if (isset($plugins["$this->type"]))
		{
			$plugin = $plugins["$this->type"];
			if(method_exists($plugin,'needStorage')) {
				$needStorage = $plugin->needStorage();
				if(method_exists($plugin,'needSpecificColumnType')) {
					$needSpecificColumnType = $plugin->needSpecificColumnType();
				}
			} else {
				$needStorage = true;
			}
		...
		if ($needStorage == "true") {
			...
			if (!$exist) {
				if($needSpecificColumnType !== false) {
					$this->_db->setQuery("ALTER TABLE #__adsmanager_ads ADD `$this->name` ".$needSpecificColumnType);
				} else {
		    ...
		    if ($this->profile == 1)
		    {
				...
				if (!$exist) {
					if($needSpecificColumnType !== false) {
						$this->_db->setQuery("ALTER TABLE #__adsmanager_profile ADD `$this->name` ".$needSpecificColumnType);
					} else {

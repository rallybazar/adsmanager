...
class AdsmanagerTableContents extends JTable
{
	...
	//contain the existing file
	var $files = null;
	//contain the name of the file_upload field
    var $filesUploadNameField = null;
	...
	function bindContent($post,$files,$conf,$model,$plugins)
    {
		...
		if ($post["selected_cats"] != null && $maxcats > 1)
		{
		...
		{ 	
			...
			else if (isset($plugins[$field->type]))
			{
				...
				if($field->type=="fileupload") {
					$filesUploadField = $field;
		                     $this->filesUploadNameField = $filesUploadField->name;
				}
			}
			else
			{
				...
				// Odstránenie slova "predam" / "predám" v rôznych formách z názvu inzerátu
				if ($field->name === 'ad_headline') {
					// Regulárny výraz ignoruje veľkosť písmen a berie do úvahy aj dĺžne
					$value = preg_replace('/\bpreda?m\b/iu', '', $value);
					// Odstráni nadbytočné medzery po vymazaní
		...
		//FILES UPLOAD
		if($this->filesUploadNameField !== null) {
			$this->data['files']= array();	
			$this->data['delfiles']= array();
			
			$fileNamefield = $this->filesUploadNameField;
			$current_files = json_decode($this->$fileNamefield);
			if ($current_files == null)
				$current_files = array();

			$file_index = 0;
			
			$deleted_files = JRequest::getVar("file_deleted_files", "" );
			$deleted_files = explode(',',$deleted_files);
			foreach($current_files as $i => $file) {
				if (in_array($file->index,$deleted_files)) {
					$this->data['delfiles'][] = $file;
				} else {
					if ($file->index > $file_index )
						$file_index = $file->index;
				}
			}
			foreach($this->data['files'] as $i => $file) {
				if (in_array($file->index,$deleted_files)) {
					if(is_file(JPATH_FILES_FOLDER."/waiting/".$file->file)) {
						JFile::delete(JPATH_FILES_FOLDER."/waiting/".$file->file);
					}
					unset($this->data['files'][$i]);
				}
			}
			
			$nb_files = count($current_files) - count($this->data['delfiles']);
			
			$nbMaxfiles = $filesUploadField->options->maxnumber;
				
			$uploader_count = JRequest::getInt('filesupload_count',0);
			
			$targetDir = JPATH_FILES_FOLDER."/uploaded";
			$dir = JPATH_FILES_FOLDER."/waiting";
			
			if (!file_exists($dir)) {
				mkdir($dir, 0755, true);
			}
			
			$orderlisttmp = explode(',',JRequest::getString('file_orderfiles',""));
			$orderlist = array();
			foreach($orderlisttmp as $value) {
				$orderlist[] = str_replace('li_file_','',$value);
			}
			
			for($i = 0 ; $i < $uploader_count && $nb_files <  $nbMaxfiles ; $i++) {
			
				$uploader_tmpname = JRequest::getString('filesupload_'.$i.'_tmpname',0);
				$uploader_id = JRequest::getString('filesupload_'.$i.'_id',0);
				$uploader_name = JRequest::getString('filesupload_'.$i.'_name',0);
				$uploader_status = JRequest::getString('filesupload_'.$i.'_status',0);
				...
				if (($uploader_status == "done")&&(file_exists($targetDir."/".$uploader_tmpname))) {
					try {
						...
						if ($error != null) {
							...
							rename($targetDir."/".$uploader_tmpname, $dir."/".$uploader_name);
							$newimg = new stdClass();
							$newimg->index = $file_index;
							$newimg->file = $uploader_name;
							$this->data['files'][] = $newimg;
							//echo $uploader_id."<br/>";
							foreach($orderlist as $key => $val) {
								if ($val == $uploader_id) {
									$orderlist[$key] = $file_index;
								}
							}
						}
					} catch (Exception $e) {
			...
			$this->data['orderfiles'] = $orderlist;
			$this->data['fields'][$filesUploadField->name] = json_encode($this->data['files']);
			$this->files = json_encode($this->data['files']);
		}
		//END FILE UPLOAD
		...
		if (($app->isAdmin() == false)&&($this->id == 0)) {
			...
			if (($app->isAdmin() == false)&&($this->update_validation)) {
				$this->published = 0;
			}
		}
	...
	function saveContent($src, $orderingFilter = '', $ignore = '')
    {
		...
		if($this->filesUploadNameField !== null) {
			$dir = JPATH_FILES_FOLDER."/waiting/";
			$dirfinal = JPATH_FILES_FOLDER."/".$contentid;
			...
			$fileNamefield = $this->filesUploadNameField;
			$current_files = json_decode($this->$fileNamefield);
	//var_dump($current_files,$this->data['files']);
			if ($current_files == null)
				$current_files = array();
			if (!is_array($current_files))
				$current_files = get_object_vars($current_files);
			
			foreach($this->data['delfiles'] as $file) {
				if(is_file(JPATH_FILES_FOLDER."/".$file->file)) {
					JFile::delete(JPATH_IMAGES_FOLDER."/".$file->file);
				}
				foreach($current_files as $key => $f) {
			...
			foreach($this->data['files'] as $file) {	
				$src  =$dir.$file->file;
				$dest =$dirfinal.$file->file;			 
				rename($src,$dest);

				$current_files[] = $file;
			}
			
			$orderlist = $this->data['orderfiles'];
			$newlistfiles = array();
			foreach($orderlist as $o) {
			...
			$this->$fileNamefield = $newlistfiles;
			
			$row = new JObject();
			$row->id = $contentid;
			$row->$fileNamefield = json_encode($newlistfiles);
			$this->$fileNamefield = $newlistfiles;
		...
		include_once(JPATH_ROOT.'/administrator/components/com_adsmanager/models/alert.php');
		$alertmodel	= new AdsmanagerModelAlert();
		$alertmodel->checkAlertsOnNewAd($this);
    }

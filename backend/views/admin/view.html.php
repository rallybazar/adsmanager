...
class AdsmanagerViewAdmin extends TView
{
	...
	function setContentsToolbar($title)
	{
		//Access Right
		$user = JFactory::getUser();
		/*$canCreate = $user->authorise('adsmanager.createcontent', 'com_adsmanager');
		$canDelete = $user->authorise('adsmanager.deletecontent', 'com_adsmanager');
		$canEdit = $user->authorise('adsmanager.editcontent', 'com_adsmanager');
		$canPublish = $user->authorise('adsmanager.publishcontent', 'com_adsmanager');
		$canUnpublish = $user->authorise('adsmanager.unpublishcontent', 'com_adsmanager');
		$canDuplicate = $user->authorise('adsmanager.duplicatecontent', 'com_adsmanager');*/
	...
	function setListToolbar($title, $type='')
	{
		$user = JFactory::getUser();

		//Access Right
		if($type != '') {
			$canCreate = $user->authorise('adsmanager.create'.$type, 'com_adsmanager');
			$canDelete = $user->authorise('adsmanager.delete'.$type, 'com_adsmanager');
			$canEdit = $user->authorise('adsmanager.edit'.$type, 'com_adsmanager');
		} else {
			$canCreate = true;
			$canDelete = true;
			$canEdit = true;
	...
	function setSimpleEditToolbar($title, $type = '')
	{
		//Access Right
		$user = JFactory::getUser();
		if($type != '') {
	...
	function _listcategories()
	{
		...
		//check if the user can access the categories
		$user = JFactory::getUser();
		if(!$user->authorise('adsmanager.accesscategory','com_adsmanager')) {
			$app->redirect('index.php', JText::_('JERROR_ALERTNOAUTHOR'), 'error');
		}
		
		//$this->setListToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_CATEGORIES"), 'category');
    ...
    function _listmails()
	{
		...
		//$this->setListToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_MAILS"), 'mail');
		$this->setListToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_MAILS"), '');
		
        JToolbarHelper::custom('send_emails', 'mail.png', 'mail_f2.png', 'JTOOLBAR_SEND_EMAILS', true);
	...
	function _editcategory()
	{
		...
		//$this->setEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_EDIT_CATEGORY"), 'category');
    ...
    function _editmail()
	{
		...
		//$this->setEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_EDIT_MAIL"), 'mail');
	...
	function _listcontents()
	{
		...
		//check if the user can access the contents
		$user = JFactory::getUser();
	...
	function _editcontent()
	{
		...
		//$this->setEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_EDIT_CONTENT"), 'content');
	...
	function _position()
	{
		...
		//check if the user can access the layout - details
		$user = JFactory::getUser();
		...
		JuloaLib::loadJqueryUI();
		//$this->setSimpleEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_CONTENT_DISPLAY"), 'positiondetails');
	...
	function _contentform()
	{
		...
		//check if the user can access the layout - content form
		$user = JFactory::getUser();
		...
		JuloaLib::loadJqueryUI();
		//$this->setSimpleEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("COM_ADSMANAGER_CONTENT_FORM"), 'content');
	...
	function _columns()
	{
		...
		//check if the user can access the layout - list
		$user = JFactory::getUser();
	...
	function _listfields()
	{
		...
		//check if the user can access the fields
		$user = JFactory::getUser();
		if(!$user->authorise('adsmanager.accessfield','com_adsmanager')) {
			$app->redirect('index.php', JText::_('JERROR_ALERTNOAUTHOR'), 'error');;
		}
		
		//$this->setListToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_FIELDS"), 'field');
	...
	function _editfield()
	{
		...
		JuloaLib::loadJqueryUI();
		//$this->setEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_EDIT_FIELD"), 'field');
        ...
        $joomla_Fields = $fieldmodel->getJoomlaUserPluginFields();
        ...
        $lists['joomlafields'] = JHTML::_('select.genericlist', $joomlaFields, 'options_common_joomla_field', 'class="inputbox" size="1"', 'value', 'text', @$field->options->joomla_field );
	...
	function _configuration()
	{
		...
		//$this->setSimpleEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_CONFIGURATION"), 'configuration');
	...
	function _searchmodule()
	{
		...
		//check if the user can access the search module
		$user = JFactory::getUser();
		...
		JuloaLib::loadJqueryUI();
		//$this->setSimpleEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_SEARCHMODULE"), 'searchmodule');
    ...
    function _searchpage()
	{
		...
		JuloaLib::loadJqueryUI();
		//$this->setSimpleEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_SEARCHPAGE"), 'searchpage');
		$this->setSimpleEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_SEARCHPAGE"), '');
	
		$model = $this->getModel();
		$conf  = $model->getSearchPageConfiguration();
		$this->assignRef('conf',$conf);
	...
	function _listfieldimages()
	{
		...
		//check if the user can access the fields images
		$user = JFactory::getUser();
	...
	function _listplugins()
	{
		...
		//check if the user can access the plugins
		$user = JFactory::getUser();
	...
	function _importad()
	{
		...
		// Nastavenie horného toolbaru (rovnako ako ostatné)
		$this->setListToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("COM_ADSMANAGER_IMPORTADS"));

		// Načítaj model
		$model = $this->getModel('Importad'); // hľadá models/importad.php

		// Ak potrebuješ config
		$confmodel = $this->getModel("Configuration");
		$this->config = $confmodel->getConfiguration();

		// Prípadne získať dáta z modelu
		$this->importData = $model->getImportData();

		// Prípadne pagination, filters atď.
		// $limit = $app->getUserStateFromRequest('global.list.limit', 'limit', $app->getCfg('list_limit'), 'int');
    ...
    function _export(){
		...
		include_once (JPATH_ROOT.'/administrator/components/com_adsmanager/models/field.php');
		...
		$fields = $fieldmodel->getAdminFields();
	...
	function _dashboard()
    {
    	...
    	$this->setListToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_DASHBOARD"));
    
    	$model = $this->getModel();

    }

	function _emailtemplates()
	{
		$app = JFactory::getApplication();

		//check if the user can access the templates
		$user = JFactory::getUser();
		...
		$this->setListToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("COM_ADSMANAGER_EMAILTEMPLATES"), '');

		$limit = $app->getUserStateFromRequest('global.list.limit', 'limit', $app->getCfg('list_limit'), 'int');
		$limitstart	= $app->getUserStateFromRequest("com_adsmanager.admin_emailtemplates.limitstart", 'limitstart', 0,'int');

		$model = $this->getModel();

		$strSearch = $app->getUserStateFromRequest( 'com_adsmanager.emailtemplate.search','search', '','word' );
		$this->strSearch = $strSearch;
		$strPublished = $app->getUserStateFromRequest( 'com_adsmanager.emailtemplate.publish','published', '','cmd' );
		$this->published = $strPublished;

		$total = $model->getNbEmailTemplates($strSearch, $strPublished);
		jimport('joomla.html.pagination');
		$pagination = new JPagination($total, $limitstart, $limit);

		$list = $model->getEmailTemplates($strSearch, $strPublished, $limit, $limitstart);
	...
	function _emailtemplate()
	{
		...
		$this->setEditToolbar(JText::_("COM_ADSMANAGER")." - ".JText::_("ADSMANAGER_EDIT_EMAIL_TEMPLATE"), '');

		$templateId = JFactory::getApplication()->input->get( 'cids', [0], '', 'array' );
		JArrayHelper::toInteger($templateId, [0]);
		...
		$EmailTemplate = $model->getEmailTemplate($id);
		if ($EmailTemplate == null) {
			$EmailTemplate = new stdClass();
			$EmailTemplate->published = 1;
		}					 

		$this->row = $EmailTemplate;

	}

	function reorderDate1( $date ) {
		$format = JText::_('ADSMANAGER_DATE_FORMAT_LC2');
	...
	function reorderDate2( $date ){
		$format = JText::_('ADSMANAGER_DATE_FORMAT_LC3');

...
<?php } ?>
	   ...
	   <?php if ($this->nbcats > 1) { ?>
		...
		<?php } else { ?>
            var catid = jQ('#categoryselect').val();

            if(catid == 0) {
                errorMSG += <?php echo json_encode(JText::_('ADSMANAGER_FORM_CATEGORY')); ?>+" : "+<?php echo json_encode(JText::_('ADSMANAGER_REGWARN_ERROR')); ?>+'\n';
				jQ('.categoryselect_cascade').css( "background", "red");
				iserror=1;
            }
...
<option value="<?php echo $user->id;?>" <?php if ($user->id == $userid) { echo "selected"; } ?>><?php echo $user->name.' ('.$user->username.')'; ?></option>
<?php } ?>
</select>
</td>
<td>&nbsp;</td>

...
class AdsmanagerModelContent extends TModel
{
	...
	function getContents($filters = null, $limitstart=null, $limit=null, $filter_order=null, $filter_order_Dir=null, $admin=0, $favorite=0, $lat=0, $lon=0, $mapFieldName='')
	{
		...
		if($enableLocationOrdering) {
			$selectByDistance = "( 3959 * acos( cos( radians('$lat') ) * 
								...
								cos( radians( a.".$mapFieldName."_lng ) - 
								radians('$lon') ) + 
								sin( radians('$lat') ) * 
								sin( radians( a.".$mapFieldName."_lat ) ) ) ) 
								AS distance, ";
	...
	function getNbContents($filters = null,$admin=0,$favorite=0,$filter_order=null,$lat=0,$lon=0,$mapFieldName='')
    {
		$filterByDistance = '';
		$enableLocationOrdering = false;
    ...
    function getLatestContents($nbcontents,$sort_type=0,$catselect="no",$rootid=null,$userselect="no")
    {
		switch($sort_type)
		{
			/* Trending (last 72 days) (3.3 beta version) */
			case 4:
				$order_sql = "ORDER BY 
					CASE 
						WHEN DATEDIFF(NOW(), a.date_created) > 72 THEN 0 
						ELSE 
							((a.views / POW(DATEDIFF(NOW(), a.date_created) + 2, 1.3)) * LOG(GREATEST(a.views, 50))) + 
							(10 * EXP(-0.04 * DATEDIFF(NOW(), a.date_created))) 
					END DESC, 
					a.date_created DESC, 
					a.id DESC ";
				break;

			 /* Popular (3.3 beta version) */
			case 3:
				$order_sql = "ORDER BY (a.views / DATEDIFF(NOW(), a.date_created)) DESC, a.date_created DESC, a.id DESC ";
				break;
		...
		switch($userselect)
		{
			...
			case "-1":
				...
				if (($adid != 0)&&($adid != -1))
				{
					$this->_db->setQuery( "SELECT userid ".
							" FROM #__adsmanager_ads ".
							" WHERE id = ".(int)$adid);
					$userid = $this->_db->loadResult();
					$prefix = "";
                    
					$user_query = $prefix." a.userid = ".(int)$userid." AND ";
				}
				break;
			default:
		...
		$sql =  " SELECT a.*,p.id as parentid,p.name as parent,c.id as catid, c.name as cat,u.username as user ".
				...
				" INNER JOIN #__adsmanager_categories as c ON adcat.catid = c.id ".
				" LEFT JOIN #__adsmanager_categories as p ON c.parent = p.id ".
				" WHERE 1 $globalfilter $rootfilter AND $cat_query $user_query c.published = 1 and a.published = 1 GROUP BY a.id $order_sql LIMIT 0, $nbcontents";
	...
	function sendExpirationEmail($content,$conf)
	{
		...
		return EmailTemplateHelperEmail::sendEmail($content, 'adsmanager_content_expiration');
	}
			...
			if (isset($idsarray) && count($idsarray) > 0) {
				foreach($idsarray as $c)
				{
					...
					if ($conf->send_email_on_expiration_to_user == 1) {
						...
						EmailTemplateHelperEmail::sendEmail($c, 'adsmanager_content_expired');
					}	
    ...
    function countFavorites($adid) {
        ...
        $favorite = $this->_db->loadObjectList();

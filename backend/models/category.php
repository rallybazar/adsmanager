...
class AdsmanagerModelCategory extends TModel
{
	...
	function getCategories($onlyPublished = true, $mode = 'admin',$rootid=0) {
		...
		$sql =  "SELECT c.* FROM #__adsmanager_categories as c ".
		...
		if (!$cachecats) {
			...
			$cachecats = $this->_db->loadObjectList();

			foreach($cachecats as &$cat) {
	...
	function getCategoriesById($onlyPublished = true, $mode = 'admin',$rootid=0) {
	...
	function getCatTree($onlyPublished = true,$getnbads = false,&$nbcontents=null, $mode = 'admin',$rootid=0) {
		...
		if ($getnbads != null) 
		{
			$cache[$cacheKey] = array($tree,$this->calc_nb_ads(0,$tree));
		} else {
			$cache[$cacheKey] = array($tree);
		}

...
class AdsmanagerControllerCategories extends TController
{
	...
	function save()
	{
		...
		if(!isset($post['usergroupsread']) || !is_array($post['usergroupsread'])) {
			$post['usergroupsread'] = array();
		}
		if(!isset($post['usergroupswrite']) || !is_array($post['usergroupswrite'])) {
			$post['usergroupswrite'] = array();
		}

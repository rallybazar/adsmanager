...
class AdsmanagerControllerContents extends TController
{
	function __construct($config= array()) {
		...
		$this->registerTask('resetviews', 'resetviews');
	}
		...
		if ($conf->auto_publish == 0)
		{
			if ($conf->send_email_on_validation_to_user == 1) {	
				...
				foreach($cid as $id) {
					...
					EmailTemplateHelperEmail::sendEmail($c, 'adsmanager_content_validated');
	...
	function save($src=null, $orderingFilter = '', $ignore = '')
	{
		...
		if (($contentid != 0)&&($conf->auto_publish == 0)) {
			if (($previouspublished == 0)&&(JRequest::getInt('published', 1))) {
				...
				EmailTemplateHelperEmail::sendEmail($content, 'adsmanager_content_validated');
		...
		switch ($task)
		{
			...
			case 'resetviews':
				...
				$app->redirect(
					"index.php?option=com_adsmanager&c=contents&task=resetViews&id={$id}",
					JText::_('COM_ADSMANAGER_RESET_VIEWS_SUCCESS'),
					'message'
				);
				break;
	...
	public function resetViews() {
		...
		if ($id) {
			...
			$query = $db->getQuery(true)
				->update($db->quoteName('#__adsmanager_ads'))
				->set($db->quoteName('views') . ' = 0')
				->where($db->quoteName('id') . ' = ' . (int)$id);
			$db->setQuery($query);
			$db->execute();
			JFactory::getApplication()->enqueueMessage(JText::_('COM_ADSMANAGER_RESET_VIEWS_SUCCESS'));
		}
